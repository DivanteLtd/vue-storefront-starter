...
export default {
  ...
  beforeMount() {
    if (this.$store.state.__DEMO_MODE__) {
      this.$store
        .dispatch('claims/check', { claimCode: 'onboardingAccepted' })
        .then(onboardingClaim => {
          if (!onboardingClaim) {
  ...
  asyncData({ store, route }) {
    ...
    return new Promise((resolve, reject) => {
      ...
      let newProductsQuery = builder()
        .query('match', 'category.name', 'Tees')
        ...
        .build()
      let coolBagsQuery = builder()
        .query('match', 'category.name', 'Women')
        .andQuery('range', 'status', { gte: 0, lt: 2 })
        .andQuery(
          'range',
          'visibility',
